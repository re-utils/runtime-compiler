...
__rtcpl_setup_aot__(($) => {
	let check_86ee428c40a56da3 = ((value) => typeof value == "object" && !!value && !Array.isArray(value) && "name" in value && "password" in value && typeof value.name == "string" && typeof value.password == "string");
	return (value) => check_86ee428c40a56da3(value);
});
//#endregion
...
function IsConstructor$2(value) {
	if (IsUndefined$2(value) || !IsFunction$2(value)) return !1;
...
function IsMultipleOf$1(dividend, divisor) {
	...
	if (!IsNumber$3(dividend) || IsInteger$2(dividend) && 1 / divisor % 1 == 0) return !0;
...
function IsClassInstance(value) {
	...
	return IsNull$2(proto) ? !1 : IsEqual$1(typeof proto.constructor, "function") && !(IsEqual$1(proto.constructor, globalThis.Object) || IsEqual$1(proto.constructor.name, "Object"));
}
...
function Clone$1(value) {
	return Metrics.clone += 1, FromValue$3(value);
}
//#endregion
...
function Create$1(hidden, enumerable, options = {}) {
	Metrics.create += 1;
	let settings = Get$3(), withOptions = Merge(enumerable, options), withHidden = settings.enumerableKind ? Merge(withOptions, hidden) : MergeHidden(withOptions, hidden);
...
function Discard(value, propertyKeys) {
	Metrics.discard += 1;
	let result = {}, descriptors = Object.getOwnPropertyDescriptors(Clone$1(value)), keysToDiscard = new Set(propertyKeys);
	for (let key of Object.keys(descriptors)) keysToDiscard.has(key) || Object.defineProperty(result, key, descriptors[key]);
	return result;
}
//#endregion
...
function Update$1(current, hidden, enumerable) {
	Metrics.update += 1;
	let settings = Get$3(), result = Clone$1(current);
...
function ImmutableAdd(type) {
	return Update$1(type, { "~immutable": !0 }, {});
}
...
function OptionalAdd(type) {
	return Update$1(type, { "~optional": !0 }, {});
}
...
function ReadonlyAdd(type) {
	return Update$1(type, { "~readonly": !0 }, {});
}
...
function _Object_$1(properties, options = {}) {
	let requiredKeys = RequiredArray(properties);
...
function Cyclic($defs, $ref, options) {
	return Create$1({ "~kind": "Cyclic" }, {
		$defs: Keys$1($defs).reduce((result, key) => ({
			...result,
...
function Infer(...args) {
	let [name, extends_] = Match$3(args, {
		2: (name, extends_) => [
			name,
			extends_,
			extends_
...
function FromArray$11(value) {
	...
	for (let item of value) FromValue$2(item);
}
...
function FromConstructor$1(value) {
	FNV1A64_OP(ByteMarker.Constructor), FromValue$2(value.toString());
}
function FromDate(value) {
	FNV1A64_OP(ByteMarker.Date), FromValue$2(value.getTime());
}
function FromFunction$1(value) {
	FNV1A64_OP(ByteMarker.Function), FromValue$2(value.toString());
}
...
function FromObject$14(value) {
	...
	for (let key of InstanceKeys(value).sort()) FromValue$2(key), FromValue$2(value[key]);
}
function FromRegExp(value) {
	FNV1A64_OP(ByteMarker.RegExp), FromString$7(value.toString());
}
...
function FromSymbol$1(value) {
	FNV1A64_OP(ByteMarker.Symbol), FromValue$2(value.toString());
}
...
function HashCode(value) {
	return Accumulator = BigInt("14695981039346656037"), FromValue$2(value), Accumulator;
}
/** Generates a FNV1A-64 non cryptographic hash of the given value */
function Hash(value) {
...
//#endregion
//#region ../../node_modules/typebox/build/type/types/identifier.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/type/types/parameter.mjs
/** Creates a Parameter type. */
function Parameter$1(...args) {
	let [name, extends_, equals] = Match$3(args, {
...
function FromTypesReduce(types) {
	return TakeLeft(types, (left, right) => FromType$23(left) ? FromTypesReduce(right) : !1, () => !0);
}
function FromTypes$4(types) {
	return IsEqual$1(types.length, 0) ? !1 : FromTypesReduce(types);
}
function FromType$23(type) {
	return IsUnion(type) ? FromTypes$4(type.anyOf) : IsLiteral(type) ? FromLiteral$6(type.const) : !1;
}
...
function Tuple$1(types, options = {}) {
	let [items, minItems, additionalItems] = [
		types,
		types.length,
		!1
	];
...
function CompositeProperty(left, right) {
	let isReadonly = IsReadonlyProperty(left, right), isOptional = IsOptionalProperty(left, right), property = ReadonlyRemove(OptionalRemove(EvaluateIntersect([left, right])));
...
function Narrow(left, right) {
	...
	return IsEqual$1(result, "left-inside") ? left : IsEqual$1(result, "right-inside") || IsEqual$1(result, "equal") ? right : Never();
}
//#endregion
...
function FromLiteralKey(key, value) {
	return IsString$3(key) || IsNumber$3(key) ? _Object_$1({ [key]: value }) : IsEqual$1(key, !1) ? _Object_$1({ false: value }) : IsEqual$1(key, !0) ? _Object_$1({ true: value }) : _Object_$1({});
}
//#endregion
...
function TryBuildRecord(types, value) {
	return IsEqual$1(StringOrNumberCheck(types), !0) ? CreateRecord(StringKey, value) : void 0;
}
function CreateProperties(types, value) {
	return types.reduce((result, left) => IsLiteral(left) && (IsString$3(left.const) || IsNumber$3(left.const)) ? {
		...result,
		[left.const]: value
	} : result, {});
}
...
const IntegerKey = `^${IntegerPattern}$`, NumberKey = `^${NumberPattern}$`, StringKey = "^.*$";
...
const DelimitedDecode = (input, result = []) => input.reduce((result, left) => IsArray$2(left) && IsEqual$1(left.length, 2) ? [...result, left[0]] : [...result, left], []), Delimited = (input) => {
...
function IndexArrayMapping(input) {
	return input.reduce((result, current) => IsEqual$1(current.length, 3) ? [...result, [current[1]]] : [...result, []], []);
}
...
const FactorIndexArray = (Type, indexArray) => indexArray.reduce((result, left) => {
	let _left = left;
	return IsEqual$1(_left.length, 1) ? IndexDeferred(result, _left[0]) : IsEqual$1(_left.length, 0) ? _Array_(result) : Unreachable();
}, Type), FactorExtends = (type, extend) => IsEqual$1(extend.length, 3) ? ConditionalDeferred(type, extend[0], extend[1], extend[2]) : type;
...
function PropertyKeyIndexMapping(input) {
	return IsInteger(input[3]) ? IntegerKey : IsNumber(input[3]) ? NumberKey : IsSymbol(input[3]) || IsString(input[3]) ? StringKey : Unreachable();
}
...
function PropertiesReduce(propertyList) {
	return propertyList.reduce((result, left) => HasPropertyKey$1(left, IntegerKey) || HasPropertyKey$1(left, NumberKey) || HasPropertyKey$1(left, StringKey) ? [result[0], Assign(result[1], left)] : [Assign(result[0], left), result[1]], [{}, {}]);
}
function PropertiesMapping(input) {
	return PropertiesReduce(input[1]);
}
function _Object_Mapping(input) {
	let [properties, patternProperties] = input;
...
function MappedReadonlyMapping(input) {
	return IsEqual$1(input.length, 2) && IsEqual$1(input[0], "-") ? "remove" : IsEqual$1(input.length, 2) && IsEqual$1(input[0], "+") || IsEqual$1(input.length, 1) ? "add" : "none";
}
...
function JsonObjectMappingReduce(propertyList) {
	return propertyList.reduce((result, left) => Assign(result, left), {});
}
...
function InterfaceDeclarationGenericMapping(input) {
	let parameters = input[2], heritage = input[3], [properties, patternProperties] = input[4], options = IsEqual$1(Keys$1(patternProperties).length, 0) ? {} : { patternProperties };
...
function InterfaceDeclarationMapping(input) {
	let heritage = input[2], [properties, patternProperties] = input[3], options = IsEqual$1(Keys$1(patternProperties).length, 0) ? {} : { patternProperties };
...
const Alpha = [...Range(97, 122), ...Range(65, 90)], NonZero = Range(49, 57), Digit = ["0", ...NonZero];
...
function TrimWhitespace(input) {
	...
	return trimmed.startsWith("/*") ? TrimWhitespace(DiscardMultilineComment(trimmed.slice(2))) : trimmed.startsWith("//") ? TrimWhitespace(DiscardLineComment(trimmed.slice(2))) : trimmed;
}
function Trim(input) {
	let trimmed = input.trimStart();
	return trimmed.startsWith("/*") ? Trim(DiscardMultilineComment(trimmed.slice(2))) : trimmed.startsWith("//") ? Trim(DiscardLineComment(trimmed.slice(2))) : trimmed;
}
//#endregion
//#region ../../node_modules/typebox/build/type/script/token/internal/optional.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/type/script/token/internal/many.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/type/script/token/unsigned_integer.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/type/script/token/integer.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/type/script/token/bigint.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/type/script/token/ident.mjs
const Initial = [
	...Alpha,
	"_",
	"$"
...
//#endregion
//#region ../../node_modules/typebox/build/type/script/token/unsigned_number.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/type/script/token/number.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/type/script/token/span.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/type/script/token/string.mjs
...
const If = (result, left, right = () => []) => result.length === 2 ? left(result) : right(), GenericParameterExtendsEquals = (input) => If(If(Ident(input), ([_0, input]) => If(Const("extends", input), ([_1, input]) => If(Type(input), ([_2, input]) => If(Const("=", input), ([_3, input]) => If(Type(input), ([_4, input]) => [[
	...
	_4,
	_5
], input])))))), ([_0, input]) => [_0, input], () => If([[], input], ([_0, input]) => [_0, input], () => [])), ([_0, input]) => [ExtendsMapping(_0), input]), Base = (input) => If(If(If(Const("(", input), ([_0, input]) => If(Type(input), ([_1, input]) => If(Const(")", input), ([_2, input]) => [[
	_0,
	_1,
	_2
], input]))), ([_0, input]) => [_0, input], () => If(Keyword(input), ([_0, input]) => [_0, input], () => If(_Object_(input), ([_0, input]) => [_0, input], () => If(Tuple(input), ([_0, input]) => [_0, input], () => If(TemplateLiteral(input), ([_0, input]) => [_0, input], () => If(Literal(input), ([_0, input]) => [_0, input], () => If(Constructor(input), ([_0, input]) => [_0, input], () => If(_Function_(input), ([_0, input]) => [_0, input], () => If(Mapped(input), ([_0, input]) => [_0, input], () => If(Options$1(input), ([_0, input]) => [_0, input], () => If(GenericCall(input), ([_0, input]) => [_0, input], () => If(Reference(input), ([_0, input]) => [_0, input], () => [])))))))))))), ([_0, input]) => [BaseMapping(_0), input]), Factor = (input) => If(If(KeyOf(input), ([_0, input]) => If(Base(input), ([_1, input]) => If(IndexArray(input), ([_2, input]) => If(Extends$1(input), ([_3, input]) => [[
	_0,
	_1,
	_2,
	_3
], input])))), ([_0, input]) => [FactorMapping(_0), input]), ExprTermTail = (input) => If(If(If(Const("&", input), ([_0, input]) => If(Factor(input), ([_1, input]) => If(ExprTermTail(input), ([_2, input]) => [[
	_0,
	_1,
	_2
	...
	_1,
	_2
], input]))), ([_0, input]) => [_0, input], () => [])))), ([_0, input]) => [ElementNamedMapping(_0), input]), ElementReadonlyOptional = (input) => If(If(Const("readonly", input), ([_0, input]) => If(Type(input), ([_1, input]) => If(Const("?", input), ([_2, input]) => [[
	_0,
	_1,
	_2
], input]))), ([_0, input]) => [ElementReadonlyOptionalMapping(_0), input]), ElementReadonly = (input) => If(If(Const("readonly", input), ([_0, input]) => If(Type(input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [ElementReadonlyMapping(_0), input]), ElementOptional = (input) => If(If(Type(input), ([_0, input]) => If(Const("?", input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [ElementOptionalMapping(_0), input]), ElementBase = (input) => If(If(ElementNamed(input), ([_0, input]) => [_0, input], () => If(ElementReadonlyOptional(input), ([_0, input]) => [_0, input], () => If(ElementReadonly(input), ([_0, input]) => [_0, input], () => If(ElementOptional(input), ([_0, input]) => [_0, input], () => If(Type(input), ([_0, input]) => [_0, input], () => []))))), ([_0, input]) => [ElementBaseMapping(_0), input]), Element = (input) => If(If(If(Const("...", input), ([_0, input]) => If(ElementBase(input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [_0, input], () => If(If(ElementBase(input), ([_0, input]) => [[_0], input]), ([_0, input]) => [_0, input], () => [])), ([_0, input]) => [ElementMapping(_0), input]), ElementList_0 = (input, result = []) => If(If(Element(input), ([_0, input]) => If(Const(",", input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => ElementList_0(input, [...result, _0]), () => [result, input]), ElementList = (input) => If(If(ElementList_0(input), ([_0, input]) => If(If(If(Element(input), ([_0, input]) => [[_0], input]), ([_0, input]) => [_0, input], () => If([[], input], ([_0, input]) => [_0, input], () => [])), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [ElementListMapping(_0), input]), Tuple = (input) => If(If(Const("[", input), ([_0, input]) => If(ElementList(input), ([_1, input]) => If(Const("]", input), ([_2, input]) => [[
	_0,
	_1,
	_2
], input]))), ([_0, input]) => [TupleMapping(_0), input]), ParameterReadonlyOptional = (input) => If(If(Ident(input), ([_0, input]) => If(Const("?", input), ([_1, input]) => If(Const(":", input), ([_2, input]) => If(Const("readonly", input), ([_3, input]) => If(Type(input), ([_4, input]) => [[
	_0,
	_1,
	_2,
	_3,
	_4
], input]))))), ([_0, input]) => [ParameterReadonlyOptionalMapping(_0), input]), ParameterReadonly = (input) => If(If(Ident(input), ([_0, input]) => If(Const(":", input), ([_1, input]) => If(Const("readonly", input), ([_2, input]) => If(Type(input), ([_3, input]) => [[
	_0,
	_1,
	_2,
	_3
], input])))), ([_0, input]) => [ParameterReadonlyMapping(_0), input]), ParameterOptional = (input) => If(If(Ident(input), ([_0, input]) => If(Const("?", input), ([_1, input]) => If(Const(":", input), ([_2, input]) => If(Type(input), ([_3, input]) => [[
	_0,
	_1,
	_2,
	_3
], input])))), ([_0, input]) => [ParameterOptionalMapping(_0), input]), ParameterType = (input) => If(If(Ident(input), ([_0, input]) => If(Const(":", input), ([_1, input]) => If(Type(input), ([_2, input]) => [[
	_0,
	_1,
	_2
], input]))), ([_0, input]) => [ParameterTypeMapping(_0), input]), ParameterBase = (input) => If(If(ParameterReadonlyOptional(input), ([_0, input]) => [_0, input], () => If(ParameterReadonly(input), ([_0, input]) => [_0, input], () => If(ParameterOptional(input), ([_0, input]) => [_0, input], () => If(ParameterType(input), ([_0, input]) => [_0, input], () => [])))), ([_0, input]) => [ParameterBaseMapping(_0), input]), Parameter = (input) => If(If(If(Const("...", input), ([_0, input]) => If(ParameterBase(input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [_0, input], () => If(If(ParameterBase(input), ([_0, input]) => [[_0], input]), ([_0, input]) => [_0, input], () => [])), ([_0, input]) => [ParameterMapping(_0), input]), ParameterList_0 = (input, result = []) => If(If(Parameter(input), ([_0, input]) => If(Const(",", input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => ParameterList_0(input, [...result, _0]), () => [result, input]), ParameterList = (input) => If(If(ParameterList_0(input), ([_0, input]) => If(If(If(Parameter(input), ([_0, input]) => [[_0], input]), ([_0, input]) => [_0, input], () => If([[], input], ([_0, input]) => [_0, input], () => [])), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [ParameterListMapping(_0), input]), _Function_ = (input) => If(If(Const("(", input), ([_0, input]) => If(ParameterList(input), ([_1, input]) => If(Const(")", input), ([_2, input]) => If(Const("=>", input), ([_3, input]) => If(Type(input), ([_4, input]) => [[
	_0,
	_1,
	_2,
	_3,
	_4
], input]))))), ([_0, input]) => [_Function_Mapping(_0), input]), Constructor = (input) => If(If(Const("new", input), ([_0, input]) => If(Const("(", input), ([_1, input]) => If(ParameterList(input), ([_2, input]) => If(Const(")", input), ([_3, input]) => If(Const("=>", input), ([_4, input]) => If(Type(input), ([_5, input]) => [[
	_0,
	_1,
	_2,
	_3,
	...
	_11,
	_12
], input]))))))))))))), ([_0, input]) => [MappedMapping(_0), input]), Reference = (input) => If(Ident(input), ([_0, input]) => [ReferenceMapping(_0), input]), Options$1 = (input) => If(If(Const("Options", input), ([_0, input]) => If(Const("<", input), ([_1, input]) => If(Type(input), ([_2, input]) => If(Const(",", input), ([_3, input]) => If(JsonObject(input), ([_4, input]) => If(Const(">", input), ([_5, input]) => [[
	_0,
	_1,
	_2,
	_3,
	_4,
	_5
], input])))))), ([_0, input]) => [OptionsMapping(_0), input]), JsonNumber = (input) => If(Number$1(input), ([_0, input]) => [JsonNumberMapping(_0), input]), JsonBoolean = (input) => If(If(Const("true", input), ([_0, input]) => [_0, input], () => If(Const("false", input), ([_0, input]) => [_0, input], () => [])), ([_0, input]) => [JsonBooleanMapping(_0), input]), JsonString = (input) => If(String$1(["\"", "'"], input), ([_0, input]) => [JsonStringMapping(_0), input]), JsonNull = (input) => If(Const("null", input), ([_0, input]) => [JsonNullMapping(_0), input]), JsonProperty = (input) => If(If(PropertyKey(input), ([_0, input]) => If(Const(":", input), ([_1, input]) => If(Json(input), ([_2, input]) => [[
	_0,
	_1,
	_2
], input]))), ([_0, input]) => [JsonPropertyMapping(_0), input]), JsonPropertyList_0 = (input, result = []) => If(If(JsonProperty(input), ([_0, input]) => If(PropertyDelimiter(input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => JsonPropertyList_0(input, [...result, _0]), () => [result, input]), JsonPropertyList = (input) => If(If(JsonPropertyList_0(input), ([_0, input]) => If(If(If(JsonProperty(input), ([_0, input]) => [[_0], input]), ([_0, input]) => [_0, input], () => If([[], input], ([_0, input]) => [_0, input], () => [])), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [JsonPropertyListMapping(_0), input]), JsonObject = (input) => If(If(Const("{", input), ([_0, input]) => If(JsonPropertyList(input), ([_1, input]) => If(Const("}", input), ([_2, input]) => [[
	_0,
	_1,
	_2
], input]))), ([_0, input]) => [JsonObjectMapping(_0), input]), JsonElementList_0 = (input, result = []) => If(If(Json(input), ([_0, input]) => If(Const(",", input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => JsonElementList_0(input, [...result, _0]), () => [result, input]), JsonElementList = (input) => If(If(JsonElementList_0(input), ([_0, input]) => If(If(If(Json(input), ([_0, input]) => [[_0], input]), ([_0, input]) => [_0, input], () => If([[], input], ([_0, input]) => [_0, input], () => [])), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [JsonElementListMapping(_0), input]), JsonArray = (input) => If(If(Const("[", input), ([_0, input]) => If(JsonElementList(input), ([_1, input]) => If(Const("]", input), ([_2, input]) => [[
	_0,
	_1,
	_2
], input]))), ([_0, input]) => [JsonArrayMapping(_0), input]), Json = (input) => If(If(JsonNumber(input), ([_0, input]) => [_0, input], () => If(JsonBoolean(input), ([_0, input]) => [_0, input], () => If(JsonString(input), ([_0, input]) => [_0, input], () => If(JsonNull(input), ([_0, input]) => [_0, input], () => If(JsonObject(input), ([_0, input]) => [_0, input], () => If(JsonArray(input), ([_0, input]) => [_0, input], () => [])))))), ([_0, input]) => [JsonMapping(_0), input]), PatternBigInt = (input) => If(Const("-?(?:0|[1-9][0-9]*)n", input), ([_0, input]) => [PatternBigIntMapping(_0), input]), PatternString = (input) => If(Const(".*", input), ([_0, input]) => [PatternStringMapping(_0), input]), PatternNumber = (input) => If(Const("-?(?:0|[1-9][0-9]*)(?:.[0-9]+)?", input), ([_0, input]) => [PatternNumberMapping(_0), input]), PatternInteger = (input) => If(Const("-?(?:0|[1-9][0-9]*)", input), ([_0, input]) => [PatternIntegerMapping(_0), input]), PatternNever = (input) => If(Const("(?!)", input), ([_0, input]) => [PatternNeverMapping(_0), input]), PatternText = (input) => If(Until_1([
	"-?(?:0|[1-9][0-9]*)n",
	".*",
	"-?(?:0|[1-9][0-9]*)(?:.[0-9]+)?",
	"-?(?:0|[1-9][0-9]*)",
	...
	_3,
	_4
], input]))))), ([_0, input]) => [InterfaceDeclarationGenericMapping(_0), input]), InterfaceDeclaration = (input) => If(If(Const("interface", input), ([_0, input]) => If(Ident(input), ([_1, input]) => If(InterfaceDeclarationHeritage(input), ([_2, input]) => If(Properties(input), ([_3, input]) => [[
	_0,
	_1,
	_2,
	_3
], input])))), ([_0, input]) => [InterfaceDeclarationMapping(_0), input]), TypeAliasDeclarationGeneric = (input) => If(If(Const("type", input), ([_0, input]) => If(Ident(input), ([_1, input]) => If(GenericParameters(input), ([_2, input]) => If(Const("=", input), ([_3, input]) => If(Type(input), ([_4, input]) => [[
	_0,
	_1,
	_2,
	_3,
	_4
], input]))))), ([_0, input]) => [TypeAliasDeclarationGenericMapping(_0), input]), TypeAliasDeclaration = (input) => If(If(Const("type", input), ([_0, input]) => If(Ident(input), ([_1, input]) => If(Const("=", input), ([_2, input]) => If(Type(input), ([_3, input]) => [[
	_0,
	_1,
	_2,
	_3
], input])))), ([_0, input]) => [TypeAliasDeclarationMapping(_0), input]), ExportKeyword = (input) => If(If(If(Const("export", input), ([_0, input]) => [[_0], input]), ([_0, input]) => [_0, input], () => If([[], input], ([_0, input]) => [_0, input], () => [])), ([_0, input]) => [ExportKeywordMapping(_0), input]), ModuleDeclarationDelimiter = (input) => If(If(If(Const(";", input), ([_0, input]) => If(Const("\n", input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [_0, input], () => If(If(Const(";", input), ([_0, input]) => [[_0], input]), ([_0, input]) => [_0, input], () => If(If(Const("\n", input), ([_0, input]) => [[_0], input]), ([_0, input]) => [_0, input], () => []))), ([_0, input]) => [ModuleDeclarationDelimiterMapping(_0), input]), ModuleDeclarationList_0 = (input, result = []) => If(If(ModuleDeclaration(input), ([_0, input]) => If(ModuleDeclarationDelimiter(input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => ModuleDeclarationList_0(input, [...result, _0]), () => [result, input]), ModuleDeclarationList = (input) => If(If(ModuleDeclarationList_0(input), ([_0, input]) => If(If(If(ModuleDeclaration(input), ([_0, input]) => [[_0], input]), ([_0, input]) => [_0, input], () => If([[], input], ([_0, input]) => [_0, input], () => [])), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [ModuleDeclarationListMapping(_0), input]), ModuleDeclaration = (input) => If(If(ExportKeyword(input), ([_0, input]) => If(If(InterfaceDeclarationGeneric(input), ([_0, input]) => [_0, input], () => If(InterfaceDeclaration(input), ([_0, input]) => [_0, input], () => If(TypeAliasDeclarationGeneric(input), ([_0, input]) => [_0, input], () => If(TypeAliasDeclaration(input), ([_0, input]) => [_0, input], () => [])))), ([_1, input]) => If(OptionalSemiColon(input), ([_2, input]) => [[
	_0,
	_1,
	_2
], input]))), ([_0, input]) => [ModuleDeclarationMapping(_0), input]), Module = (input) => If(If(ModuleDeclaration(input), ([_0, input]) => If(ModuleDeclarationList(input), ([_1, input]) => [[_0, _1], input])), ([_0, input]) => [ModuleMapping(_0), input]), Script$1 = (input) => If(If(Module(input), ([_0, input]) => [_0, input], () => If(GenericType(input), ([_0, input]) => [_0, input], () => If(Type(input), ([_0, input]) => [_0, input], () => []))), ([_0, input]) => [ScriptMapping(_0), input]);
//#endregion
...
function EncodeBigInt(right, pattern) {
	return EncodeTypes(right, `${pattern}-?(?:0|[1-9][0-9]*)n`);
}
...
function EncodeType(type, right, pattern) {
	return IsEnum$1(type) ? EncodeEnum(type.enum, right, pattern) : IsInteger(type) ? EncodeInteger(right, pattern) : IsLiteral(type) ? EncodeLiteral(type.const, right, pattern) : IsBigInt(type) ? EncodeBigInt(right, pattern) : IsBoolean(type) ? EncodeBoolean(right, pattern) : IsNumber(type) ? EncodeNumber(right, pattern) : IsString(type) ? EncodeString(right, pattern) : IsTemplateLiteral(type) ? EncodeTemplateLiteral(type.pattern, right, pattern) : IsTemplateLiteralDeferred(type) ? EncodeTemplateLiteralDeferred(type.parameters[0], right, pattern) : IsUnion(type) ? EncodeUnion(type.anyOf, right, pattern) : "(?!)";
}
...
function ExtendsAny(inferred, left, right) {
	return IsInfer(right) ? ExtendsRight(inferred, left, right) : IsAny(right) || IsUnknown(right) ? ExtendsTrue(inferred) : ExtendsUnion$1(inferred);
}
//#endregion
...
function ExtendsConstructor(inferred, parameters, returnType, right) {
	return IsAny(right) || IsUnknown(right) ? ExtendsTrue(inferred) : IsConstructor(right) ? Match$1(ExtendsParameters(inferred, parameters, right.parameters), (inferred) => ExtendsReturnType(inferred, returnType, right.instanceType), () => ExtendsFalse()) : ExtendsFalse();
}
//#endregion
...
function ExtendsFunction(inferred, parameters, returnType, right) {
	return IsAny(right) || IsUnknown(right) ? ExtendsTrue(inferred) : IsFunction(right) ? Match$1(ExtendsParameters(inferred, parameters, right.parameters), (inferred) => ExtendsReturnType(inferred, returnType, right.returnType), () => ExtendsFalse()) : ExtendsFalse();
}
//#endregion
//#region ../../node_modules/typebox/build/type/extends/integer.mjs
function ExtendsInteger(inferred, left, right) {
	return IsInteger(right) || IsNumber(right) ? ExtendsTrue(inferred) : ExtendsRight(inferred, left, right);
}
//#endregion
...
function ExtractInferredProperties(keys, properties) {
	return keys.reduce((result, key) => key in properties && IsExtendsTrueLike(properties[key]) ? {
		...result,
...
function ExtendsPropertiesComparer(inferred, left, right) {
	let properties = {};
	for (let rightKey of Keys$1(right)) properties[rightKey] = rightKey in left ? ExtendsProperty({}, left[rightKey], right[rightKey]) : IsOptional(right[rightKey]) ? IsInfer(right[rightKey]) ? ExtendsTrue(Assign(inferred, { [right[rightKey].name]: right[rightKey].extends })) : ExtendsTrue(inferred) : ExtendsFalse();
	let checked = Values(properties).every((result) => IsExtendsTrueLike(result)), extracted = checked ? ExtractInferredProperties(Keys$1(properties), properties) : {};
...
function Reversed(types) {
	let first = types.length > 0 ? types[0] : void 0;
...
function ExtendsUndefined(inferred, left, right) {
	return IsVoid(right) || IsUndefined(right) ? ExtendsTrue(inferred) : ExtendsRight(inferred, left, right);
}
//#endregion
...
function ExtendsUnknown(inferred, left, right) {
	return IsInfer(right) ? ExtendsRight(inferred, left, right) : IsAny(right) || IsUnknown(right) ? ExtendsTrue(inferred) : ExtendsFalse();
}
//#endregion
...
function FromRef$9(stack, context, ref) {
	return stack.includes(ref) ? !0 : FromType$21([...stack, ref], context, context[ref]);
}
...
function FromTypes$3(stack, context, types) {
	return TakeLeft(types, (left, right) => FromType$21(stack, context, left) ? !0 : FromTypes$3(stack, context, right), () => !1);
}
...
function ResolveCandidateKeys(context, keys) {
	return keys.reduce((result, left) => left in context ? CyclicCheck([left], context, context[left]) ? [...result, left] : result : Unreachable$1(), []);
}
...
function FromTypes$2(context, types, result) {
	return types.reduce((result, left) => FromType$20(context, left, result), result);
}
...
function FromTypes$1(types) {
	return types.reduce((result, left) => [...result, FromType$19(left)], []);
}
...
function Compare(left, right) {
	let extendsCheck = [IsUnknown(left) ? ExtendsFalse() : Extends({}, left, right), IsUnknown(left) ? ExtendsTrue({}) : Extends({}, right, left)];
	return IsExtendsTrueLike(extendsCheck[0]) && IsExtendsTrueLike(extendsCheck[1]) ? "equal" : IsExtendsTrueLike(extendsCheck[0]) && IsExtendsFalse(extendsCheck[1]) ? "left-inside" : IsExtendsFalse(extendsCheck[0]) && IsExtendsTrueLike(extendsCheck[1]) ? "right-inside" : "disjoint";
}
//#endregion
//#region ../../node_modules/typebox/build/type/engine/evaluate/broaden.mjs
function BroadFilter(type, types) {
	return types.filter((left) => Compare(type, left) !== "right-inside");
}
function IsBroadestType(type, types) {
	return IsEqual$1(types.some((left) => {
		let result = Compare(type, left);
...
function BroadenTypes(types) {
	return types.reduce((result, left) => IsObject(left) ? [...result, left] : IsNever(left) ? result : BroadenType(left, result), []);
}
/** Broadens a set of types and returns either the most broad type, or union or disjoint types. */
function Broaden(types) {
	let flattened = Flatten(BroadenTypes(types));
...
function Distribute(zipped) {
	return zipped.reduce((result, left) => IsEqual$1(left[0], !0) ? Cross(result, Expand(left[1])) : Cross(result, [left[1]]), [[]]);
}
function DistributeArguments(parameters, arguments_, expression) {
	let zippedArguments = ZipDistributionArray(arguments_, BuildDistributionArray(parameters, CollectDistributionNames(expression)));
	return IsDeferred(expression) && IsEqual$1(expression.action, "Conditional") || IsDeferred(expression) && IsEqual$1(expression.action, "Mapped") ? Distribute(zippedArguments) : [arguments_];
}
//#endregion
...
function AssertArgumentExtends(name, type, extends_) {
	...
	throw Error(`Argument for parameter ${name} does not satisfy constraint`, { cause });
}
function BindArgument(context, state, name, extends_, type) {
	let instantiatedArgument = InstantiateType(context, state, type);
	return AssertArgumentExtends(name, instantiatedArgument, extends_), Assign(context, { [name]: instantiatedArgument });
}
...
function CallImmediate(context, state, target, parameters, expression, arguments_) {
	let returnTypes = CallDistributed(context, state, target, parameters, expression, DistributeArguments(parameters, arguments_, expression));
...
const CapitalizeMapping = (input) => input[0].toUpperCase() + input.slice(1), LowercaseMapping = (input) => input.toLowerCase(), UncapitalizeMapping = (input) => input[0].toLowerCase() + input.slice(1), UppercaseMapping = (input) => input.toUpperCase();
...
function CollapseIntersectProperties(left, right) {
	let leftKeys = Keys$1(left).filter((key) => !HasPropertyKey$1(right, key)), rightKeys = Keys$1(right).filter((key) => !HasPropertyKey$1(left, key)), sharedKeys = Keys$1(left).filter((key) => HasPropertyKey$1(right, key)), leftProperties = leftKeys.reduce((result, key) => ({
		...result,
		[key]: left[key]
	}), {}), rightProperties = rightKeys.reduce((result, key) => ({
		...result,
		[key]: right[key]
	}), {}), sharedProperties = sharedKeys.reduce((result, key) => ({
		...result,
...
function FromIntersect$10(types) {
	return types.reduce((result, left) => CollapseIntersectProperties(result, FromType$16(left)), {});
}
//#endregion
...
const integerKeyPattern = /* @__PURE__ */ RegExp("^(?:0|[1-9][0-9]*)$");
...
function FromUnion$10(types) {
	return types.reduce((result, left) => [...result, ...FromType$15(left)], []);
}
//#endregion
...
function IndexElementsWithIndexer(types, indexer) {
	return types.reduceRight((result, right, index) => IsExtendsTrueLike(Extends({}, Literal$1(index), indexer)) ? [right, ...result] : result, []);
}
...
function FromPropertyKeys(keys) {
	return keys.reduce((result, left) => IsLiteralValue(left) ? [...result, Literal$1(ConvertToIntegerKey(left))] : Unreachable$1(), []);
}
...
function FromUnion$9(types) {
	return types.reduce((result, left) => [...result, ...FromType$11(left)], []);
}
...
function MappedVariant(context, state, identifier, variant, as, property) {
	let variantContext = Assign(context, { [identifier.name]: variant }), canonicalAs = CanonicalAs(InstantiateType(variantContext, state, as)), instantiatedProperty = InstantiateType(variantContext, state, property);
...
function MappedObjects(properties) {
	return properties.reduce((result, left) => [...result, _Object_$1(left)], []);
}
...
function InstantiateModule(context, options) {
	let cyclicCandidates = CyclicCandidates(context), instantiatedCyclics = InstantiateCyclics(context, cyclicCandidates), instantiatedNonCyclics = InstantiateNonCyclics(context, cyclicCandidates);
	return Update$1({
		...instantiatedCyclics,
...
function FromKeys$1(properties, keys) {
	return Keys$1(properties).reduce((result, key) => keys.includes(key) ? result : {
		...result,
		[key]: properties[key]
	}, {});
}
...
function FromCyclic$8(defs, ref) {
	let partial = FromType$9(CyclicTarget(defs, ref));
...
function FromKeys(properties, keys) {
	return Keys$1(properties).reduce((result, key) => keys.includes(key) ? Assign(result, { [key]: properties[key] }) : result, {});
}
...
function FromCyclic$7(defs, ref) {
	let partial = FromType$7(CyclicTarget(defs, ref));
	return Cyclic(Assign(defs, { [ref]: partial }), ref);
}
//#endregion
...
function FromCyclic$6(defs, ref) {
	let partial = FromType$6(CyclicTarget(defs, ref));
	return Cyclic(Assign(defs, { [ref]: partial }), ref);
}
//#endregion
...
function SpreadElement(type) {
	return IsRest(type) ? IsTuple(type.items) ? RestSpread(type.items.items) : IsInfer(type.items) || IsRef$1(type.items) ? [type] : [Never()] : [type];
}
function RestSpread(types) {
	return types.reduce((result, left) => [...result, ...SpreadElement(left)], []);
}
//#endregion
//#region ../../node_modules/typebox/build/type/engine/instantiate.mjs
function CanInstantiate(types) {
	return TakeLeft(types, (left, right) => IsRef$1(left) ? !1 : CanInstantiate(right), () => !0);
}
...
function IsItems(schema) {
	return HasPropertyKey$1(schema, "items") && (IsSchema(schema.items) || IsArray$2(schema.items) && schema.items.every((value) => IsSchema(value)));
}
...
function HasUnevaluatedFromUnknown(value) {
	return IsArray$2(value) ? HasUnevaluatedFromArray(value) : IsObject$2(value) ? HasUnevaluatedFromObject(value) : !1;
}
var CheckContext = class {
	...
	constructor(callback) {
		super(), this.callback = callback;
	}
	AddError(error) {
		return this.callback(error), !1;
	}
}, AccumulatedErrorContext = class extends ErrorContext {
	constructor() {
		super((error) => this.errors.push(error)), this.errors = [];
	}
	AddError(error) {
		return this.errors.push(error), !1;
	}
	GetErrors() {
		return this.errors;
	}
...
function ErrorRefine(_stack, context, schemaPath, instancePath, schema, value) {
	return EveryAll(schema["~refine"], 0, (refinement, index) => refinement.check(value) || context.AddError({
		keyword: "~refine",
		schemaPath,
		instancePath,
		params: {
...
function ErrorAdditionalItems(stack, context, schemaPath, instancePath, schema, value) {
	return IsValid$4(schema) ? value.every((item, index) => {
		let nextSchemaPath = `${schemaPath}/additionalItems`, nextInstancePath = `${instancePath}/${index}`;
...
function ErrorAdditionalProperties(stack, context, schemaPath, instancePath, schema, value) {
	let regexp = new RegExp(GetPropertiesPattern(schema)), additionalProperties = [];
	return EveryAll(Keys$1(value), 0, (key, _index) => {
		let nextSchemaPath = `${schemaPath}/additionalProperties`, nextInstancePath = `${instancePath}/${key}`, nextContext = new AccumulatedErrorContext(), isAdditionalProperty = regexp.test(key) || ErrorSchemaPushStack(stack, nextContext, nextSchemaPath, nextInstancePath, schema.additionalProperties, value[key]) && context.AddKey(key);
		return isAdditionalProperty || additionalProperties.push(key), isAdditionalProperty;
	}) || context.AddError({
...
function ErrorAllOf(stack, context, schemaPath, instancePath, schema, value) {
	let failedContexts = [], results = schema.allOf.reduce((result, schema, index) => {
		let nextSchemaPath = `${schemaPath}/allOf/${index}`, nextContext = new AccumulatedErrorContext(), isSchema = ErrorSchema(stack, nextContext, nextSchemaPath, instancePath, schema, value);
		return isSchema || failedContexts.push(nextContext), isSchema ? [...result, nextContext] : result;
	}, []), isAllOf = IsEqual$1(results.length, schema.allOf.length) && context.Merge(results);
	return isAllOf || failedContexts.forEach((failed) => failed.GetErrors().forEach((error) => context.AddError(error))), isAllOf;
}
//#endregion
...
function ErrorAnyOf(stack, context, schemaPath, instancePath, schema, value) {
	let failedContexts = [], results = schema.anyOf.reduce((result, schema, index) => {
		let nextContext = new AccumulatedErrorContext(), isSchema = ErrorSchema(stack, nextContext, `${schemaPath}/anyOf/${index}`, instancePath, schema, value);
		return isSchema || failedContexts.push(nextContext), isSchema ? [...result, nextContext] : result;
	}, []), isAnyOf = IsGreaterThan$1(results.length, 0) && context.Merge(results);
	return isAnyOf || failedContexts.forEach((failed) => failed.GetErrors().forEach((error) => context.AddError(error))), isAnyOf || context.AddError({
		keyword: "anyOf",
		schemaPath,
		instancePath,
		params: {}
...
function BuildDependencies(stack, context, schema, value) {
	return Or(IsEqual(Member(Keys(value), "length"), Constant(0)), ReduceAnd(Entries$2(schema.dependencies).map(([key, schema]) => {
		...
		return Or(notKey, IsArray$2(schema) ? ((schema) => ReduceAnd(schema.map((key) => HasPropertyKey(value, Constant(key)))))(schema) : isSchema);
	})));
}
function CheckDependencies(stack, context, schema, value) {
	let isLength = IsEqual$1(Keys$1(value).length, 0), isEvery = Every$1(Entries$2(schema.dependencies), 0, ([key, schema]) => !HasPropertyKey$1(value, key) || (IsArray$2(schema) ? schema.every((key) => HasPropertyKey$1(value, key)) : CheckSchema(stack, context, schema, value)));
	return isLength || isEvery;
}
function ErrorDependencies(stack, context, schemaPath, instancePath, schema, value) {
	let isLength = IsEqual$1(Keys$1(value).length, 0), isEvery = EveryAll(Entries$2(schema.dependencies), 0, ([key, schema]) => {
		let nextSchemaPath = `${schemaPath}/dependencies/${key}`;
...
function ErrorDependentRequired(_stack, context, schemaPath, instancePath, schema, value) {
	let isLength = IsEqual$1(Keys$1(value).length, 0), isEveryEntry = EveryAll(Entries$2(schema.dependentRequired), 0, ([key, keys]) => !HasPropertyKey$1(value, key) || EveryAll(keys, 0, (dependency) => HasPropertyKey$1(value, dependency) || context.AddError({
...
function ErrorDependentSchemas(stack, context, schemaPath, instancePath, schema, value) {
	let isLength = IsEqual$1(Keys$1(value).length, 0), isEvery = EveryAll(Entries$2(schema.dependentSchemas), 0, ([key, schema]) => {
		let nextSchemaPath = `${schemaPath}/dependentSchemas/${key}`;
...
function ErrorIf(stack, context, schemaPath, instancePath, schema, value) {
	let thenSchema = IsThen(schema) ? schema.then : !0, elseSchema = IsElse(schema) ? schema.else : !0, trueContext = new AccumulatedErrorContext(), isIf = ErrorSchema(stack, trueContext, `${schemaPath}/if`, instancePath, schema.if, value) ? ErrorSchema(stack, trueContext, `${schemaPath}/then`, instancePath, thenSchema, value) || context.AddError({
		keyword: "if",
		schemaPath,
		instancePath,
	...
	return isIf && context.Merge([trueContext]), isIf;
}
//#endregion
...
function ErrorItemsSized(stack, context, schemaPath, instancePath, schema, value) {
	return EveryAll(schema.items, 0, (schema, index) => {
		let nextSchemaPath = `${schemaPath}/items/${index}`, nextInstancePath = `${instancePath}/${index}`;
...
function ErrorItemsUnsized(stack, context, schemaPath, instancePath, schema, value) {
	return EveryAll(value, IsPrefixItems(schema) ? schema.prefixItems.length : 0, (element, index) => ErrorSchemaPushStack(stack, context, `${schemaPath}/items`, `${instancePath}/${index}`, schema.items, element) && context.AddIndex(index));
}
...
function ErrorMaxContains(stack, context, schemaPath, instancePath, schema, value) {
	let minContains = IsMinContains(schema) ? schema.minContains : 1;
...
function ErrorOneOf(stack, context, schemaPath, instancePath, schema, value) {
	let failedContexts = [], passingSchemas = [], passedContexts = schema.oneOf.reduce((result, schema, index) => {
		let nextContext = new AccumulatedErrorContext(), isSchema = ErrorSchema(stack, nextContext, `${schemaPath}/oneOf/${index}`, instancePath, schema, value);
		return isSchema && passingSchemas.push(index), isSchema || failedContexts.push(nextContext), isSchema ? [...result, nextContext] : result;
	}, []), isOneOf = IsEqual$1(passedContexts.length, 1) && context.Merge(passedContexts);
	return !isOneOf && IsEqual$1(passingSchemas.length, 0) && failedContexts.forEach((failed) => failed.GetErrors().forEach((error) => context.AddError(error))), isOneOf || context.AddError({
		keyword: "oneOf",
		schemaPath,
		instancePath,
...
function ErrorPatternProperties(stack, context, schemaPath, instancePath, schema, value) {
	return EveryAll(Entries$2(schema.patternProperties), 0, ([pattern, schema]) => {
		let nextSchemaPath = `${schemaPath}/patternProperties/${pattern}`, regexp = new RegExp(pattern, "u");
		return EveryAll(Entries$2(value), 0, ([key, value]) => {
			let nextInstancePath = `${instancePath}/${key}`;
...
function ErrorPrefixItems(stack, context, schemaPath, instancePath, schema, value) {
	return IsEqual$1(value.length, 0) || EveryAll(schema.prefixItems, 0, (schema, index) => {
		let nextSchemaPath = `${schemaPath}/prefixItems/${index}`, nextInstancePath = `${instancePath}/${index}`;
		return IsLessEqualThan$1(value.length, index) || ErrorSchemaPushStack(stack, context, nextSchemaPath, nextInstancePath, schema, value[index]) && context.AddIndex(index);
	});
}
//#endregion
...
function ErrorProperties(stack, context, schemaPath, instancePath, schema, value) {
	...
	return EveryAll(Entries$2(schema.properties), 0, ([key, schema]) => {
		let nextSchemaPath = `${schemaPath}/properties/${key}`, nextInstancePath = `${instancePath}/${key}`, isProperty = () => !HasPropertyKey$1(value, key) || ErrorSchemaPushStack(stack, context, nextSchemaPath, nextInstancePath, schema, value[key]) && context.AddKey(key);
		return IsExactOptional(required, key) ? isProperty() : InexactOptionalCheck(value, key) || isProperty();
	});
}
//#endregion
...
function ErrorPropertyNames(stack, context, schemaPath, instancePath, schema, value) {
	let propertyNames = [];
	return EveryAll(Keys$1(value), 0, (key, _index) => {
		let nextInstancePath = `${instancePath}/${key}`, nextSchemaPath = `${schemaPath}/propertyNames`, isPropertyName = ErrorSchema(stack, new AccumulatedErrorContext(), nextSchemaPath, nextInstancePath, schema.propertyNames, key);
		return isPropertyName || propertyNames.push(key), isPropertyName;
	}) || context.AddError({
...
function ErrorRef(stack, context, _schemaPath, instancePath, schema, value) {
	let target = stack.Ref(schema) ?? !1, nextContext = new AccumulatedErrorContext(), result = IsSchema(target) && ErrorSchema(stack, nextContext, "#", instancePath, target, value);
	return result && context.Merge([nextContext]), result || nextContext.GetErrors().forEach((error) => context.AddError(error)), result;
}
//#endregion
...
function ErrorRequired(_stack, context, schemaPath, instancePath, schema, value) {
	...
	return EveryAll(schema.required, 0, (key) => {
		let hasKey = HasPropertyKey$1(value, key);
		return hasKey || requiredProperties.push(key), hasKey;
	}) || context.AddError({
		keyword: "required",
		schemaPath,
		instancePath,
...
function ErrorUnevaluatedItems(stack, context, schemaPath, instancePath, schema, value) {
	let indices = context.GetIndices(), unevaluatedItems = [];
	return EveryAll(value, 0, (item, index) => {
		let nextContext = new AccumulatedErrorContext(), isEvaluatedItem = (indices.has(index) || ErrorSchema(stack, nextContext, schemaPath, instancePath, schema.unevaluatedItems, item)) && context.AddIndex(index);
		return isEvaluatedItem || unevaluatedItems.push(index), isEvaluatedItem;
	}) || context.AddError({
...
function ErrorUnevaluatedProperties(stack, context, schemaPath, instancePath, schema, value) {
	let keys = context.GetKeys(), unevaluatedProperties = [];
	return EveryAll(Entries$2(value), 0, ([key, prop]) => {
		let nextContext = new AccumulatedErrorContext(), isEvaluatedProperty = keys.has(key) || ErrorSchema(stack, nextContext, schemaPath, instancePath, schema.unevaluatedProperties, prop) && context.AddKey(key);
		return isEvaluatedProperty || unevaluatedProperties.push(key), isEvaluatedProperty;
	}) || context.AddError({
...
function ErrorUniqueItems(_stack, context, schemaPath, instancePath, schema, value) {
	if (!IsValid(schema)) return !0;
	let set = /* @__PURE__ */ new Set(), duplicateItems = value.reduce((result, value, index) => {
		let hash = Hash(value);
		return set.has(hash) ? [...result, index] : (set.add(hash), result);
	}, []);
...
function BuildSchema(stack, context, schema, value) {
	...
	if (HasArrayKeywords(schema)) {
		...
		let reduced = ReduceAnd(constraints), guarded = Or(Not(IsArray$1(value)), reduced);
	...
	if (HasStringKeywords(schema)) {
		...
		let reduced = ReduceAnd(constraints), guarded = Or(Not(IsString$2(value)), reduced);
...
function FromArray$7(schema, base, ref) {
	return schema.reduce((result, item) => {
		let match = FromValue$1(item, base, ref);
		return IsUndefined$2(match) ? result : match;
	}, void 0);
}
function FromObject$7(schema, base, ref) {
	return Keys$1(schema).reduce((result, key) => {
		let match = FromValue$1(schema[key], base, ref);
		return IsUndefined$2(match) ? result : match;
	}, void 0);
}
...
function DynamicRef(root, base, dynamicRef, dynamicAnchors) {
	...
	if (!IsUndefined$2(fragmentTarget)) return !IsSchemaObject(fragmentTarget) || !IsDynamicAnchor(fragmentTarget) || new URL(dynamicRef.$dynamicRef, "http://unknown/").hash.startsWith("#/") ? fragmentTarget : dynamicAnchors.find((anchor) => anchor.$dynamicAnchor === fragmentTarget.$dynamicAnchor) ?? fragmentTarget;
}
//#endregion
...
function Check$1(...args) {
	let [context, schema, value] = Match$3(args, {
...
function Check(...args) {
	let [context, type, value] = Match$3(args, {
...
function FromArray$6(context, type, value) {
	return IsArray$2(value) ? value.map((value) => FromType$5(context, type.items, value)) : value;
}
//#endregion
...
function EvaluateIntersection(context, type) {
	let additionalProperties = HasPropertyKey$1(type, "unevaluatedProperties") ? { additionalProperties: type.unevaluatedProperties } : {}, evaluated = Evaluate(Instantiate(context, type));
...
function FromObject$6(context, type, value) {
	...
	let additionalProperties = GetAdditionalProperties(type);
	for (let key of Keys$1(value)) {
		if (HasPropertyKey$1(type.properties, key)) {
			value[key] = FromType$5(context, type.properties[key], value[key]);
			continue;
		}
		if (IsBoolean$3(additionalProperties) && IsEqual$1(additionalProperties, !0) || IsSchema$1(additionalProperties) && Check(context, additionalProperties, value[key])) {
...
function FromRecord$5(context, type, value) {
	...
	let additionalProperties = GetAdditionalProperties(type), [recordPattern, recordValue] = [new RegExp(RecordPattern(type)), RecordValue(type)];
	for (let key of Keys$1(value)) {
...
function FromObjectInstance(value) {
	let result = {};
	for (let key of Keys$1(value)) IsUnsafePropertyKey(key) || (result[key] = Clone(value[key]));
	for (let key of Symbols(value)) result[key] = Clone(value[key]);
	return result;
}
...
function FromUnion$5(context, type, value) {
	for (let schema of UnionPrioritySort(type.anyOf)) {
		let clean = FromType$5(context, schema, Clone(value));
...
function FromBoolean$5(value) {
	return IsEqual$1(value, !0) ? Ok(BigInt(1)) : Ok(BigInt(0));
}
const bigintPattern = /^-?(0|[1-9]\d*)n$/, decimalPattern = /^-?(0|[1-9]\d*)\.\d+$/, integerPattern = /^-?(0|[1-9]\d*)$/;
...
function FromBigInt$5(value) {
	return IsEqual$1(value, BigInt(0)) ? Ok(!1) : IsEqual$1(value, BigInt(1)) ? Ok(!0) : void 0;
}
function FromNumber$4(value) {
	return IsEqual$1(value, 0) ? Ok(!1) : IsEqual$1(value, 1) ? Ok(!0) : void 0;
}
function FromString$5(value) {
	return IsEqual$1(value.toLowerCase(), "false") ? Ok(!1) : IsEqual$1(value.toLowerCase(), "true") ? Ok(!0) : IsEqual$1(value, "0") ? Ok(!1) : IsEqual$1(value, "1") ? Ok(!0) : void 0;
}
function TryBoolean(value) {
	return IsBigInt$2(value) ? FromBigInt$5(value) : IsBoolean$3(value) ? Ok(value) : IsNumber$3(value) ? FromNumber$4(value) : IsNull$2(value) ? Ok(!1) : IsString$3(value) ? FromString$5(value) : IsUndefined$2(value) ? Ok(!1) : void 0;
}
//#endregion
...
function FromBoolean$4(value) {
	return IsEqual$1(value, !1) ? Ok(null) : void 0;
}
...
const maxBigInt = BigInt(2 ** 53 - 1), minBigInt = BigInt(-(2 ** 53 - 1));
...
function TryString(value) {
	return IsBigInt$2(value) || IsBoolean$3(value) || IsNumber$3(value) ? Ok(value.toString()) : IsNull$2(value) ? Ok("null") : IsString$3(value) ? Ok(value) : IsUndefined$2(value) ? Ok("") : void 0;
}
//#endregion
...
function FromBoolean$2(value) {
	return IsEqual$1(value, !1) ? Ok(void 0) : void 0;
}
...
function FromUnion$4(context, type, value) {
	...
	let selected = type.anyOf.map((type) => FromType$4(context, type, Clone(value))).find((value) => Check(context, type, value));
...
function FromLiteralString(_context, type, value) {
	let result = TryString(value);
	return IsOk(result) && IsEqual$1(type.const, result.value) ? result.value : value;
}
function FromLiteral$1(context, type, value) {
	return IsEqual$1(type.const, value) ? value : IsLiteralBigInt(type) ? FromLiteralBigInt(context, type, value) : IsLiteralBoolean(type) ? FromLiteralBoolean(context, type, value) : IsLiteralNumber(type) ? FromLiteralNumber(context, type, value) : IsLiteralString(type) ? FromLiteralString(context, type, value) : Unreachable$1();
}
//#endregion
//#region ../../node_modules/typebox/build/value/convert/from_null.mjs
function FromNull$1(_context, _type, value) {
	let result = TryNull(value);
	return IsOk(result) ? result.value : value;
}
//#endregion
...
function FromAdditionalProperties(context, entries, additionalProperties, value) {
	let keys = Keys$1(value);
	for (let [regexp, _] of entries) for (let key of keys) regexp.test(key) || (value[key] = FromType$4(context, additionalProperties, value[key]));
	return value;
}
//#endregion
...
function FromProperties(context, type, value) {
	let entries = EntriesRegExp(type.properties), keys = Keys$1(value);
	for (let [regexp, property] of entries) for (let key of keys) !regexp.test(key) || IsOptionalUndefined(property, key, value) || (value[key] = FromType$4(context, property, value[key]));
...
function FromPatternProperties(context, type, value) {
	let entries = EntriesRegExp(type.patternProperties), keys = Keys$1(value);
	for (let [regexp, schema] of entries) for (let key of keys) regexp.test(key) && (value[key] = FromType$4(context, schema, value[key]));
	return HasPropertyKey$1(type, "additionalProperties") && IsObject$2(type.additionalProperties) ? FromAdditionalProperties(context, entries, type.additionalProperties, value) : value;
}
...
function FromDefault$1(type, value) {
	return IsUndefined$2(value) ? IsFunction$2(type.default) ? type.default() : Clone(type.default) : value;
}
//#endregion
...
function FromObject$3(context, type, value) {
	if (!IsObject$2(value)) return value;
	let knownPropertyKeys = Keys$1(type.properties);
	for (let key of knownPropertyKeys) IsUndefined$2(FromType$3(context, type.properties[key], value[key])) && (IsOptional(type.properties[key]) || !HasPropertyKey$1(type.properties[key], "default")) || (value[key] = FromType$3(context, type.properties[key], value[key]));
	if (!IsAdditionalProperties(type) || IsBoolean$3(type.additionalProperties)) return value;
	for (let key of Keys$1(value)) knownPropertyKeys.includes(key) || (value[key] = FromType$3(context, type.additionalProperties, value[key]));
	return value;
}
//#endregion
...
function FromRecord$3(context, type, value) {
	if (!IsObject$2(value)) return value;
	let [recordKey, recordValue] = [new RegExp(RecordPattern(type)), RecordValue(type)];
	for (let key of Keys$1(value)) recordKey.test(key) && IsDefault(recordValue) && (value[key] = FromType$3(context, recordValue, value[key]));
	if (!IsAdditionalProperties(type)) return value;
	for (let key of Keys$1(value)) recordKey.test(key) || (value[key] = FromType$3(context, type.additionalProperties, value[key]));
	return value;
}
//#endregion
...
function FromTuple$3(context, schema, value) {
	if (!IsArray$2(value)) return value;
	let [items, max] = [schema.items, Math.max(schema.items.length, value.length)];
	for (let i = 0; i < max; i++) i < items.length && (value[i] = FromType$3(context, items[i], value[i]));
	return value;
}
//#endregion
...
function FromUnion$3(context, schema, value) {
	for (let inner of schema.anyOf) {
		let result = FromType$3(context, inner, Clone(value));
...
function FromType$3(context, type, value) {
	let defaulted = IsDefault(type) ? FromDefault$1(type, value) : value;
...
function Callback(direction, context, type, value) {
	return IsCodec(type) ? IsEqual$1(direction, "Decode") ? Decode$7(context, type, value) : Encode$7(context, type, value) : value;
}
//#endregion
...
function FromCyclic$2(direction, context, type, value) {
	return value = FromType$2(direction, {
		...context,
		...type.$defs
	}, Ref$1(type.$ref), value), Callback(direction, context, type, value);
}
//#endregion
...
function NonMatchingInterior(value, interiors) {
	for (let interior of interiors) if (!IsDeepEqual$1(value, interior)) return interior;
	return value;
}
function Decode$5(direction, context, type, value) {
	...
	let interiors = type.allOf.map((schema) => FromType$2(direction, context, schema, Clean(schema, Clone(value))));
...
function Encode$5(direction, context, type, value) {
	if (IsEqual$1(type.allOf.length, 0)) return Callback(direction, context, type, value);
	let exterior = Callback(direction, context, type, value), interiors = type.allOf.map((schema) => FromType$2(direction, context, schema, Clean(schema, Clone(exterior))));
	return interiors.every((result) => IsObject$2(result)) ? MergeInteriors(interiors) : NonMatchingInterior(exterior, interiors);
}
...
function Decode$4(direction, context, type, value) {
	...
	for (let key of Keys$1(type.properties)) !HasPropertyKey$1(value, key) || IsOptionalUndefined(type.properties[key], key, value) || (value[key] = FromType$2(direction, context, type.properties[key], value[key]));
	return Callback(direction, context, type, value);
}
function Encode$4(direction, context, type, value) {
	let exterior = Callback(direction, context, type, value);
	if (!IsObjectNotArray$1(exterior)) return exterior;
	for (let key of Keys$1(type.properties)) !HasPropertyKey$1(exterior, key) || IsOptionalUndefined(type.properties[key], key, exterior) || (exterior[key] = FromType$2(direction, context, type.properties[key], exterior[key]));
	return exterior;
}
...
function Decode$3(direction, context, type, value) {
	if (!IsObjectNotArray$1(value)) return Unreachable$1();
	let regexp = new RegExp(RecordPattern(type));
	for (let key of Keys$1(value)) regexp.test(key) || Unreachable$1(), value[key] = FromType$2(direction, context, RecordValue(type), value[key]);
	return Callback(direction, context, type, value);
}
function Encode$3(direction, context, type, value) {
	...
	if (!IsObjectNotArray$1(exterior)) return exterior;
	let regexp = new RegExp(RecordPattern(type));
	for (let key of Keys$1(exterior)) regexp.test(key) && (exterior[key] = FromType$2(direction, context, RecordValue(type), exterior[key]));
	return exterior;
}
...
function Decode$1(direction, context, type, value) {
	for (let schema of UnionPrioritySort(type.anyOf, 1)) if (Check(context, schema, value)) return Callback(direction, context, type, FromType$2(direction, context, schema, value));
	return value;
}
function Encode$1(direction, context, type, value) {
	let exterior = Callback(direction, context, type, value);
	for (let schema of UnionPrioritySort(type.anyOf, -1)) {
		let variant = FromType$2(direction, context, schema, Clone(exterior));
		if (Check(context, schema, variant)) return variant;
	}
	return exterior;
}
...
//#endregion
//#region ../../node_modules/typebox/build/value/codec/has.mjs
...
function FromObject$1(context, type) {
	return IsCodec(type) || Keys$1(type.properties).some((key) => FromType$1(context, type.properties[key]));
}
function FromRecord$1(context, type) {
	return IsCodec(type) || FromType$1(context, RecordValue(type));
}
function FromRef$1(context, type) {
	return visited.has(type.$ref) ? !1 : (visited.add(type.$ref), IsCodec(type) || HasPropertyKey$1(context, type.$ref) && FromType$1(context, context[type.$ref]));
}
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/error.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_default.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_array.mjs
function FromArray(context, type) {
	...
	let length = IsMinItems(type) ? type.minItems : 0;
	return Array.from({ length }, () => FromType(context, type.items));
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_async_iterator.mjs
async function* CreateAsyncIterator() {}
function FromAsyncIterator(_context, _type) {
	return CreateAsyncIterator();
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_base.mjs
function FromBase(_context, type) {
	return type.Create();
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_bigint.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_boolean.mjs
function FromBoolean(_context, _type) {
	return !1;
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_constructor.mjs
function FromConstructor(context, type) {
	let instanceType = FromType(context, type.instanceType);
	return class {
		constructor() {
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_cyclic.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_enum.mjs
function FromEnum(context, type) {
	return FromType(context, EnumToUnion(type));
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_function.mjs
function FromFunction(context, type) {
	let returnType = FromType(context, type.returnType);
	return () => returnType;
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_integer.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_intersect.mjs
function FromIntersect(context, type) {
	return FromType(context, Evaluate(Instantiate(context, type)));
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_iterator.mjs
function* CreateIterator() {}
function FromIterator(_context, _type) {
	return CreateIterator();
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_literal.mjs
function FromLiteral(_context, type) {
	return type.const;
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_never.mjs
function FromNever(_context, type) {
	throw new CreateError(type, "Cannot create TNever types");
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_null.mjs
function FromNull(_context, _type) {
	return null;
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_number.mjs
function FromNumber(_context, type) {
	return IsExclusiveMinimum(type) && IsNumber$3(type.exclusiveMinimum) ? type.exclusiveMinimum + 1 : IsMinimum(type) ? type.minimum : 0;
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_object.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_promise.mjs
function FromPromise(context, type) {
	return Promise.resolve(FromType(context, type.item));
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_record.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_ref.mjs
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_string.mjs
function FromString(_context, type) {
	...
	let minLength = IsMinLength(type) ? type.minLength : 0;
	return "".padEnd(minLength);
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_symbol.mjs
function FromSymbol(_context, _type) {
	return Symbol();
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_template_literal.mjs
function FromTemplateLiteral(context, type) {
	let decoded = TemplateLiteralDecode(type.pattern);
...
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_tuple.mjs
function FromTuple(context, type) {
	return Array.from({ length: type.minItems }, (_, i) => FromType(context, type.items[i]));
}
//#endregion
//#region ../../node_modules/typebox/build/value/create/from_union.mjs
